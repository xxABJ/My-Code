...
class PlayerSelectionMenu(Menu):
    def __init__(self, game):
        Menu.__init__(self, game)
        self.topx, self.topy = self.game.DISPLAY_W, self.game.DISPLAY_H * 0.25
        self.bottomx, self.bottomy = self.game.DISPLAY_W, self.game.DISPLAY_H * 0.75
        self.mid_topx, self.mid_topy = self.topx/2, self.topy/2
        self.mid_bottomx, self.mid_bottomy = self.bottomx/2, self.bottomy/2
        self.topsurface = pygame.Surface((self.topx, self.topy))
        self.bottomsurface = pygame.Surface((self.bottomx, self.bottomy))
        self.playerchoice_surface = pygame.Surface((self.bottomx * 0.25, self.bottomy))

        self.state = "Start Game Player Selection"
        #self.state = "Start Game Player settings"
        self.one_playertopx, self.one_playertopy = self.game.DISPLAY_W*0.20, self.mid_topy
        self.two_playertopx, self.two_playertopy = self.game.DISPLAY_W*0.40, self.mid_topy
        self.three_playertopx, self.three_playertopy = self.game.DISPLAY_W*0.60, self.mid_topy
        self.four_playertopx, self.four_playertopy = self.game.DISPLAY_W*0.80, self.mid_topy
        self.selected = "One Player"
        #self.selected = "Two Player"
        #self.selected = "Three Player"
        #self.selected = "Four Player"
        

        self.one_playerselected = False
        self.two_playerselected = False
        self.three_playerselected = False
        self.four_playerselected = False

    def draw_playerchoices(self):
        pass

    def prepare_menu(self):
        self.game.window.blit(self.topsurface, (0, 0))
        self.game.window.blit(self.bottomsurface, (0, int(self.game.DISPLAY_H * 0.25)))
