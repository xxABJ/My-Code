...
while get_entity_type() == Entities.Grass or Entities.Bush or Entities.Carrots or Grounds.Soil or Grounds.Turf:
	...
	if detection_of_unharvested_crop_during_main_harvesting_rotaion == 0:
		...
		for main_harvesting_rotation in range(main_harvesting_rotation):
			...
			# Checks to see if the farmland has an "even side length" amount of blocks and is on a specific "Starting Point" block. If true then, starts an "even-sided farmland" route.
			if (total_side_length % 2 == 0) and (bl_block or br_block or tr_block or tl_block):
				...
				if (bl_block):
					...
					for number_of_required_set_rotations in range(number_of_required_set_rotations): # # # # also need function for when reaches end point and this loop and number_of_required_set_rotations is still not done or just moving to end points
						#1. A loop that moves the drone in the North direction until it reaches the very top.
						#2. Moves the drone in the East direction after 'checking for turf -> checking for ability to harvest (if True) -> harvesting -> planting grass -> Updating the direction system'.
						#3. Moves the drone in the East direction after 'checking for soil -> checking for ability to harvest (if True) -> harvesting -> tilling the ground -> planting grass'.
						#4. Moves the drone in the East direction after 'checking for ability to harvest (if False) -> harvesting -> tilling the ground -> planting grass'.
						#5.
						#6.
						#7.	
							
						#1.
						for north in range(total_side_length_positional - y):
							#1.
							#2.
							#3.
