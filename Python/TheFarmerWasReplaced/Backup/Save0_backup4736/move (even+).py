...
# Assigning names to sets ("Starting Points").
...
while get_entity_type() == Entities.Grass or Entities.Bush or Entities.Carrots or Grounds.Soil or Grounds.Turf:
	...
	if detection_of_unharvested_crop_during_main_harvesting_rotaion == 0:
		...
		# The main harvesting rotation loop.
		for main_harvesting_rotation in range(main_harvesting_rotation):
			#1. Checks to see if the farmland has an "even side length" amount of blocks and is on a specific "Starting Point" block. If true then, starts an "even-sided farmland" route.
			#2. odd
			#3. direction
			
			#1.
			if (total_side_length % 2 == 0) and (bl_block or br_block or tr_block or tl_block):
				...
				if (bl_block):
					...
					for number_of_required_set_rotations in range(number_of_required_set_rotations): # # # # also need function for when reaches end point and this loop and number_of_required_set_rotations is still not done or just moving to end points
						...
						#2. Moves the drone in the East direction after 'checking for turf -> checking for ability to harvest (if True) -> harvesting -> planting grass', then updating the "Set and Direction system".
						#3. Moves the drone in the East direction after 'checking for soil -> checking for ability to harvest (if True) -> harvesting -> tilling the ground -> planting grass', then updating the "Set and Direction system".
						#4. Moves the drone in the East direction after 'checking for ability to harvest (if False)', then updating the "Set and Direction system".
						#5. A loop that moves the drone in the South direction until it reaches the bottom of the farmland.
						#6. Checks to see the if the drone is able to move in the East direction, which also indicates the end of a "Set-rotation".
						#7.	When (#6) is false, the drone does not move, indicating the end of a "Full-rotation".
							
						#1.
						for north_moves in range(total_side_length_positional - y):
				...
				#2.	
					...
					for number_of_required_set_rotations in range(number_of_required_set_rotations): # # # # also need function for when reaches end point and this loop and number_of_required_set_rotations is still not done or just moving to end points
						...
						#2. Moves the drone in the North direction after 'checking for turf -> checking for ability to harvest (if True) -> harvesting -> planting grass', then updating the "Set and Direction system".
						#3. Moves the drone in the North direction after 'checking for soil -> checking for ability to harvest (if True) -> harvesting -> tilling the ground -> planting grass', then updating the "Set and Direction system".
						#4. Moves the drone in the North direction after 'checking for ability to harvest (if False)', then updating the "Set and Direction system".
						#5. A loop that moves the drone in the East direction until it reaches the right side of the farmland.
						#6. Checks to see the if the drone is able to move in the North direction, which also indicates the end of a "Set-rotation".
						#7.	When (#6) is false, the drone does not move, which also indicates the end of a "Full-rotation".
							
						#1.
						for west_moves in range(x):
